...
export interface TransferenciaPDFData {
  schoolName: string
  schoolCnpj?: string
...
function replaceTemplateVars(text: string, data: TransferenciaPDFData): string {
  return text
    ...
    .replace(/\{\{data_transferencia\}\}/g, data.dataTransferencia)
    .replace(/\{\{nome_mae\}\}/g, data.nomeMae ?? '')
}

export function gerarTransferenciaPDF(data: TransferenciaPDFData): jsPDF {
  const doc = new jsPDF()
  ...
  doc.setFontSize(14)
  doc.setFont('helvetica', 'bold')
  doc.text('DECLARAÇÃO DE TRANSFERÊNCIA', pageWidth / 2, y, { align: 'center' })
  y += 12
  ...
  if (data.templateDeclaracao) {
    ...
    bodyText =
      ...
      `matrícula ${data.alunoMatricula}, nascido(a) em ${data.alunoDataNascimento}, ` +
      `foi transferido(a) no ano letivo de ${data.anoLetivo}, da turma ${data.turmaCodigo} - ${data.turmaSerie}, ` +
      `turno ${data.turmaTurno}, neste estabelecimento de ensino, ` +
      `conforme documentação apresentada. Data da transferência: ${data.dataTransferencia}.`
  }
